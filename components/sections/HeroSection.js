...
const Hero = ({data}) => {
  ...
  return (
    <div className={`3xl:h-[768px] bg-center 2xl:bg-left bg-no-repeat bg-cover sm:h-[690px] h-[90vh]`} style={{backgroundImage: `url("${data?.props?.backgroundImage}")`,backgroundColor:data?.styling?.backgroundColor
}} >
      <div className="max-w-[1120px] mx-auto h-full px-3 xl:px-0  flex-grow flex flex-col justify-center">
        <h1 className="font-bold text-white text-2xl sm:text-3xl md:text-4xl leading-[40px] sm:leading-[46px] md:leading-[65px] text-center lg:text-start max-w-[750px] mx-auto lg:mx-0 w-full " style={{color:data?.styling?.textColor}}>
          {data?.content?.title}
        </h1>
